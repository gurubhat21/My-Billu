      ...
      build: (context) {
        switch (template) {
          case InvoiceTemplate.modern:
            return _buildModernTemplate(bill, businessName, businessAddress, businessPhone, businessGstin, isA5, bk, logoImage);
          case InvoiceTemplate.classic:
            return _buildClassicTemplate(bill, businessName, businessAddress, businessPhone, businessGstin, isA5, bk, logoImage);
          case InvoiceTemplate.minimal:
            return _buildMinimalTemplate(bill, businessName, businessAddress, businessPhone, businessGstin, isA5, bk, logoImage);
          case InvoiceTemplate.gstInvoice:
            return _buildGstInvoiceTemplate(bill, businessName, businessAddress, businessPhone, businessGstin, isA5, bk, logoImage);
        }
      },
    ));
    return pdf.save();
...
Generated by My Billu - Smart Billing Software''';
  ...
  static pw.Widget _buildModernTemplate(Bill bill, String bName, String bAddr, String bPhone, String bGstin, bool isA5, _BankInfo bk, pw.ImageProvider? logo) {
  ...
  static pw.Widget _buildClassicTemplate(Bill bill, String bName, String bAddr, String bPhone, String bGstin, bool isA5, _BankInfo bk, pw.ImageProvider? logo) {
    final fs = isA5 ? 0.8 : 1.0;
    return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
  ...
  static pw.Widget _buildMinimalTemplate(Bill bill, String bName, String bAddr, String bPhone, String bGstin, bool isA5, _BankInfo bk, pw.ImageProvider? logo) {
    final fs = isA5 ? 0.8 : 1.0;
    return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
      // Logo at top
      if (logo != null) ...[
  ...
  static pw.Widget _buildItemsTable(Bill bill, PdfColor headerBg, PdfColor headerText, double fs) {
    ...
    final serialStyle = pw.TextStyle(fontSize: 7.5 * fs, color: PdfColors.grey700, fontStyle: pw.FontStyle.italic);
    final descStyle = pw.TextStyle(fontSize: 7.5 * fs, color: PdfColors.grey600);
    final pad = pw.EdgeInsets.symmetric(horizontal: 5 * fs, vertical: 3 * fs);

    // Check if any items have serial numbers or descriptions
    final hasSerials = bill.items.any((i) => i.serialNumber != null && i.serialNumber!.isNotEmpty);
    final hasDescs = bill.items.any((i) => i.description != null && i.description!.isNotEmpty);
    ...
    return pw.Table(
      ...
      columnWidths: {
        ...
        if (hasSerials) 2: const pw.FlexColumnWidth(1.5),  // Serial No
        if (hasDescs) (hasSerials ? 3 : 2): const pw.FlexColumnWidth(2),  // Description
        (hasSerials && hasDescs ? 4 : hasSerials || hasDescs ? 3 : 2): const pw.FlexColumnWidth(1.2),  // Price
        (hasSerials && hasDescs ? 5 : hasSerials || hasDescs ? 4 : 3): const pw.FlexColumnWidth(0.8),  // Qty
        (hasSerials && hasDescs ? 6 : hasSerials || hasDescs ? 5 : 4): const pw.FlexColumnWidth(0.7),  // GST%
        (hasSerials && hasDescs ? 7 : hasSerials || hasDescs ? 6 : 5): const pw.FlexColumnWidth(1),    // GST
        (hasSerials && hasDescs ? 8 : hasSerials || hasDescs ? 7 : 6): const pw.FlexColumnWidth(1.3),  // Total
      },
      children: [
        // Header
        pw.TableRow(
          ...
          children: [
            ...
            if (hasSerials) pw.Padding(padding: pad, child: pw.Text('Serial No', style: headerStyle)),
            if (hasDescs) pw.Padding(padding: pad, child: pw.Text('Description', style: headerStyle)),
        ...
        ...bill.items.asMap().entries.map((e) {
          ...
          return pw.TableRow(
            ...
            children: [
              ...
              pw.Padding(padding: pad, child: pw.Text(item.itemName, style: cellStyle)),
              if (hasSerials) pw.Padding(padding: pad, child: pw.Text(
                item.serialNumber ?? '', style: serialStyle)),
              if (hasDescs) pw.Padding(padding: pad, child: pw.Text(
                item.description ?? '', style: descStyle)),
  ...
  static pw.Widget _buildGstItemsTable(Bill bill, double fs) {
    ...
    // Build headers
    final headers = <String>['no.', 'Particulars'];
    if (hasSerials) headers.add('Serial No');
    if (hasDescs) headers.add('Description');
    headers.addAll(['Qty', 'Rate', 'Amount', 'GST%', 'GST Total', 'Total']);

    // Build column widths
    final colWidths = <int, pw.TableColumnWidth>{};
    int idx = 0;
    colWidths[idx++] = pw.FixedColumnWidth(22 * fs);
    colWidths[idx++] = pw.FlexColumnWidth(hasSerials || hasDescs ? 2 : 3);
    if (hasSerials) colWidths[idx++] = pw.FlexColumnWidth(1.5);
    if (hasDescs) colWidths[idx++] = pw.FlexColumnWidth(1.8);
    colWidths[idx++] = pw.FixedColumnWidth(30 * fs);
    colWidths[idx++] = pw.FixedColumnWidth(50 * fs);
    colWidths[idx++] = pw.FixedColumnWidth(55 * fs);
    colWidths[idx++] = pw.FixedColumnWidth(28 * fs);
    colWidths[idx++] = pw.FixedColumnWidth(50 * fs);
    colWidths[idx++] = pw.FixedColumnWidth(55 * fs);
      ...
      children: [
        ...
        ...bill.items.asMap().entries.map((e) {
          ...
          final cells = <pw.Widget>[
            _tCell('${e.key + 1}', fs),
            pw.Padding(padding: pad, child: pw.Text(item.itemName, style: pw.TextStyle(fontSize: 9 * fs))),
          ];
          if (hasSerials) cells.add(pw.Padding(padding: pad, child: pw.Text(item.serialNumber ?? '', style: serialStyle)));
          if (hasDescs) cells.add(pw.Padding(padding: pad, child: pw.Text(item.description ?? '', style: pw.TextStyle(fontSize: 7.5 * fs, color: PdfColors.grey600))));
          cells.addAll([
        ...
        pw.TableRow(
          ...
          children: [
            ...
            if (hasSerials) _tCell('', fs),
            if (hasDescs) _tCell('', fs),
            _tCell('', fs), // Qty
            _tCell('', fs), // Rate
  ...
  static pw.Widget _buildGstInvoiceTemplate(Bill bill, String bName, String bAddr, String bPhone, String bGstin, bool isA5, _BankInfo bk, pw.ImageProvider? logo) {
